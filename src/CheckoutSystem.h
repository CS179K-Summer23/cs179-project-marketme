...
bool containsIgnoreCase(const std::string & mainString,
...
bool isSoonToExpire(const string & expirationDate,
  const string & currentDate) {

  struct tm tm_expire, tm_current;
  strptime(expirationDate.c_str(), "%Y-%m-%d", & tm_expire);
  strptime(currentDate.c_str(), "%Y-%m-%d", & tm_current);

  time_t expireTime = mktime( & tm_expire);
  time_t currentTime = mktime( & tm_current);

  double secondsInAMonth = 30 * 24 * 60 * 60;
  return difftime(expireTime, currentTime) <= secondsInAMonth;
}
...
int checkAge() {
  ...
  if (scannedCode.length() == 4) {} else if (scannedCode.length() == 16) {
        ...
        for (const auto & code: coupons["coupons"]) {
          if (code["code"] == coupon) {
            if (isCouponValid(code, total)) {
              double discountRate = stoi(coupon.substr(coupon.size() - 2)) / 100.0;
              double eligibleTotal = 0.0;

              if (coupon.substr(0, 7) == "SPECIAL") {
                for (const auto & item: cartItems) {
                  string expirationDate = item.second.getExpirationDate();
                  if (isSoonToExpire(expirationDate, getCurrentDate())) {
                    eligibleTotal += item.second._price;
                  }
                }
              } else {
                eligibleTotal = total;
              ...
              discountValue = std::min(code["max_discount"].get < double > (), eligibleTotal * discountRate);
              total -= discountValue;
